...
/// (NavMenu.razor): un'unica query string con "query" + un offset indipendente per
/// ciascuna vista ("allOffset", "airportsOffset", "flightsOffset").
///
...
/// Tenerli tutti e tre nella query string, invece che in uno stato lato server, mantiene
...
/// costruendo il prossimo URL con BuildUrl — cosi' gli altri due offset "viaggiano" intatti
...
public static class SearchNavigation
{
    ...
    public sealed record State(string Query, int Limit, int AllOffset, int AirportsOffset, int FlightsOffset)
    {
